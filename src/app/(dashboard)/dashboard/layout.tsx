...
const Layout = async ({ children }: LayoutProps) => {
  ...
  const unseenAppointmentCount = (
    (await fetchRedis(
      "smembers",
      `user:${user.id}:incoming_appointment`
    )) as Appointment[]
  ).length;
  return (
    <div className="w-full flex h-screen">
      ...
      <div className="hidden md:flex h-full w-full max-w-xs grow flex-col gap-y-5 overflow-y-auto border-r border-gray-200 bg-white px-6 min-w-max">
